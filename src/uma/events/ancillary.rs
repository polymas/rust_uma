...
use super::{DecodeError, common::PolymarketAncillary};
...
/// Bounds the value at the first non-digit character instead of the next
/// comma. `market_id` templates are not always comma-terminated — real Neg
/// Risk ancillary data has been observed as `market_id: 907474 res_data: ...`
/// (space, not comma), which would otherwise pull the whole trailing res_data
/// text into the "value" and fail to parse as a number.
