...
- **生产故障修复：edge 节点一旦满员就永远回不到均衡**（`console/registry.rs`）。
  ...
  公开列表里（新连接进不来），也不再被自动均衡看见（旧连接放不掉），只能永远
  卡在 N/N 等人工 release。2026-09-10 生产上 `ubuntu3` 卡在 1000/1000 三个多
  小时，其余 8 台各 ~260。现在均衡的候选集是新的 `sheddable()`（`Serving` +
  `Full`，摘流中的仍排除，那是另一套节流在放），搬运的目标集合仍然只看
  `serving()`（满员/摘流中的接不住）；面板上满员节点也会正常显示"超出 N"。
- **公开列表按连接数升序改为"同一档内随机"**：跟最空节点差在 `balance_spread_pct`
  （默认 5%）以内的节点算同一档，档内每次请求随机排序，档外仍按连接数升序垫底。
  下游契约不变（还是取 `nodes[0]`），但一波重连不会再全压到唯一"最空"的那一台
  上——`ubuntu3` 就是这样一次被顶到满员的。列表带 `max-age` 缓存，同一份缓存内
  顺序仍然固定，这只是把冲顶摊薄，真正兜底的是上面那条"满员也能被摘回来"。

## v0.7.0（2026-09-09，83501e7）
- **自动均衡换判据：跟最空的节点比，而不是跟平均值比**（`console/registry.rs`）。
  老规则是"连接数超过服务中节点平均值 ×1.1 且超出 ≥20 才释放"，在 9 台的集群
  里一台空节点只把均值拉低 1/9，谁都算不上超载——2026-09-09 一台 edge 重启后
  卡在 20 个连接、其余 8 台各 380，`balance_excess` 最大只有 17，自动均衡一次
  都没触发。现在的目标是**最忙节点 ≤ 最空节点 ×1.05**：超出这条线 ≥5 时，每次
  心跳只让当前最忙的那一个节点以 1012 释放一批（一次最多搬掉差距的一半，避免
  把最空的那台顶成最忙的），下游重拉列表落到最空的节点，如此反复直到收敛。
  设置项 `balance_threshold_pct` 换成 `balance_spread_pct`（默认 5），
  `balance_min_excess` 默认 20 → 5；`Settings` 加 `#[serde(default)]`，旧
  `nodes.json` 不会让控制台起不来。新增回归测试模拟 9 台里 1 台空节点的分布，
  16 轮心跳收敛到每台 340（老规则下一次都不会触发）。
- **公开节点列表改按当前连接数升序**（原本按剩余容量降序）：`nodes[0]` 就是最空
  的那台，下游按契约取 `nodes[0]` 即可自动填补最空的节点。各节点 `max_clients`
  相同时与原顺序一致；`weight`（剩余容量）字段保留不变，按权重挑的下游不受影响。
  契约文本（`/llms.txt` 集群段）同步更新。
